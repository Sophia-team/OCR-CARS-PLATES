...
class Compress(object):
    def __init__(self, quality_lower=0, quality_upper=1, p=0.3):
        self.quality_lower = quality_lower
        self.quality_upper = quality_upper
        self.p = p
        self.augmenter = albu.JpegCompression(quality_lower=quality_lower, quality_upper=self.quality_upper, p=self.p)

    def __call__(self, item):
        item['image'] = self.augmenter(image=item['image'])['image']
        return item

class Blur(object):
    def __init__(self, blur_limit=10, p=0.3):
        self.blur_limit = blur_limit
        self.p = p
        self.augmenter = albu.Blur(p=self.p, blur_limit=self.blur_limit)
...
class ScaleToZeroOne(object):
    def __call__(self, item):
        item['image'] = np.float32(item['image'] / 255.)
        return item
